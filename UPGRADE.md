...
## 1. Confirm platform support and namespace

Version 4 requires PHP 8.1+ and Laravel 10–13. Laravel 8/9 and PHP 8.0 are no longer supported.

All package classes moved from the previous vendor namespace to `Arm092\Translation`. This is the primary breaking change. Update application imports, type hints, container bindings, aliases, tests, and explicit provider registration. For example:
...
The package name remains `arm092/laravel-translation`. Classes under the previous namespace are not retained as compatibility aliases.
...
```shell
php artisan vendor:publish --provider="Arm092\Translation\TranslationServiceProvider" --tag=config --force
```
...
```shell
php artisan vendor:publish --provider="Arm092\Translation\TranslationServiceProvider" --tag=assets --force
```

The public URLs remain `/vendor/translation/css/main.css` and `/vendor/translation/js/app.js`. The default manager now uses a responsive full-width workspace with top navigation, refreshed tables/forms/status states, and one normalized caret per select. Remove application overrides that rely on the old Vue component or Mix manifest. If you maintain customized published views, compare the layout, navigation, language/translation screens, and form partials with v4 before deployment; published application views do not update automatically.
...
- Replace every application reference to the previous package namespace with `Arm092\Translation`.
...
## 5. Choose the inline frontend

No frontend-mode configuration is required:

- no Livewire or Livewire 3 keeps the Blade/Alpine/Fetch editor;
- `livewire/livewire:^4.0` automatically enables the Livewire component;
- Livewire is optional and remains a Composer suggestion.
...
In Livewire mode, the configured authorization gate is checked during component mount and again before every save. The package stylesheet remains loaded, while package `app.js` is omitted to prevent duplicate Alpine initialization.

## 6. Database-driver checks
