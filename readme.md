...
In fallback mode, filtering uses Fetch and saves use the existing package POST endpoint with Laravel's CSRF token and same-origin credentials. In Livewire mode, one namespaced table component owns search, group filtering, page size, results, and pagination; `wire:model.live.debounce.300ms` updates search results without page navigation or focus loss. Namespaced child editor components lock locale/group/key identifiers against client mutation, validate the same payload rules as the HTTP endpoint, and write through the same driver-level action. Both save paths dispatch `TranslationAdded`, preserve language isolation, and invalidate driver caches.
...
Version 4 uses Blade, Tailwind CSS 4, and Vite 8. The fallback frontend uses the package's Alpine.js 3, `/vendor/translation/js/app.js`, and the small `/vendor/translation/js/translation-search.js` Fetch asset. When Livewire 4 is active, Livewire supplies Alpine and its request runtime, and neither package JavaScript asset is loaded; this avoids starting Alpine twice and lets Livewire update only its translation-table component. The package stylesheet remains `/vendor/translation/css/main.css` in both modes.
